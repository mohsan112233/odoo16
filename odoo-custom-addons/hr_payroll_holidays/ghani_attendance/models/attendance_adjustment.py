            ...
            if rec.check_in:
                ...
                if rec.late_emp > 0.0:
                    ...
                    duplicate = self.env["hr.leave"].search_count(
                        [('employee_id', '=', rec.employee_id.id), ('date_from', '=', rec.check_in.date()),
                         ])
                    if count_late >= 0 and not duplicate:
                        print('work', count_late, rec.late_emp)
                        ...
                        for type in list_leave_type:
                            ...
                            late_check_in = self.env['late.check_in'].search(
                                ['&', ('employee_id', '=', rec.employee_id.id), ('date', '=', rec.check_in.date())])
                            # print(duplicate,';;;;;')
                            # count_late = self.env["hr.attendance"].search_count(
                            #     [('employee_id', '=', rec.employee_id.id), ('checkin_date', '>=', rec.from_date),
                            ...
                            if rec.late_emp <= 4 and rec.late_emp >= 2 and total_leave >= 0.5:
                                ...
                                self.env['late.check_in'].create({
                                    ...
                                    'amount': 0.5,
                                    'description': f'Deduct from leave {leave_id.name}',
                                })
                                print('/..............work')
                                id.action_validate()
                                break
                                ...
                                self.env['late.check_in'].sudo().create({
                                    ...
                                    'attendance_id': rec.id,
                                    'amount': 0.25,
                                    'description': f'Deduct from {leave_id.name}',
                                })
                                print('/..............work')
                                id.action_validate()
                                break
                        ...
                        if count_late >= 4 and rec.late_emp <= 4 and rec.late_emp >= 2 and not duplicate:
                            if late_check_in:
                                late_check_in.unlink()
                        ...
                        elif count_late >= 4 and rec.late_emp <= 2 and not duplicate:
                            if late_check_in:
                                late_check_in.unlink()
                            ...
                            id.action_validate()
                        elif count_late >= 4 and rec.late_emp > 4 and not duplicate:
                            if late_check_in:
                                late_check_in.unlink()

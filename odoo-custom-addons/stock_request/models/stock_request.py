...
class StockRequest(models.Model):
    ...
    location_id = fields.Many2one(
        states={"draft": [("readonly", False)]}, readonly=True,compute="onchange_location",store=True
    )
    ...
    def check_order_location(self):
        # for i in self.order_id.stock_request_ids:
        #     i.expected_date = self.order_id.expected_date
        #     i.picking_policy = self.order_id.picking_policy
        #     i.warehouse_id = self.order_id.warehouse_id.id
        #     i.location_id = self.order_id.location_id.id
        #     i.procurement_group_id = self.order_id.procurement_group_id.id
        #     i.company_id = self.order_id.company_id.id
        #     i.state = self.order_id.state
    ...
    def onchange_location(self):
        print('pcjkdnckdnzcnujncjdbhbdyu')
        for i in self.order_id.stock_request_ids:
            i.expected_date = self.order_id.expected_date
            i.picking_policy = self.order_id.picking_policy
            i.warehouse_id = self.order_id.warehouse_id.id

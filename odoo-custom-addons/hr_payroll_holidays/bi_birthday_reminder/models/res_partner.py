...
class res_partner(models.Model):
    ...
    def _cron_birthday_reminder(self):
        ...
        for partner in employees:
            emp = self.env['hr.attendance'].search(
                [('employee_id', '=', partner.id), ('checkin_date', '=', datetime.now().date())])
            ...
            if emp:
                template_id = self.env['ir.model.data']._xmlid_to_res_id(
                    'bi_birthday_reminder.email_template_edi_birthday_reminder', raise_if_not_found=False)
                ...
                if template_browse:
                    ...
                    values['email_to'] = partner.work_email
